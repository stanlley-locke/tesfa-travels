    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-[#6b7b65] selection:text-white">
      ...
      <section className="relative h-[65vh] flex flex-col items-center justify-center overflow-hidden bg-[#111111]">
        <div className="absolute inset-0 z-0">
          <Image src="https://images.unsplash.com/photo-1512453979798-5ea266f8880c?q=80&w=2070&auto=format&fit=crop" alt="Tesfa Travels Global Presence" fill className="object-cover opacity-30" priority />
      ...
      <section className="relative py-24 px-6 bg-[#fbfbfb] overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 flex items-center justify-center opacity-40 pointer-events-none overflow-hidden">
          <Earth mapBrightness={6} dark={0.1} baseColor={[0.5, 0.6, 0.5]} glowColor={[1, 1, 1]} className='w-[800px] md:w-[1000px] opacity-40' />
        </div>
            ...
            {branches.map((loc, idx) => (
              <div key={idx} className="group flex flex-col bg-white shadow-sm border border-neutral-100 hover:shadow-2xl hover:border-neutral-200 transition-all duration-700 rounded-none overflow-hidden relative">
                ...
                <div className="relative h-64 w-full overflow-hidden bg-neutral-100">
                  <Image src={loc.image} alt={loc.city} fill className="transition-transform duration-700 object-cover group-hover:scale-105" />
                  <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors duration-500" />
                  <div className="absolute top-6 left-6 flex items-center gap-2">
                    <span className="bg-white text-black font-mono text-[10px] tracking-widest font-bold px-4 py-2 rounded-none shadow-sm">{loc.type}</span>
                  </div>
                ...
                <div className="p-8 lg:p-10 flex flex-col h-full bg-white relative z-20">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 h-full">
                    <div className="space-y-6">
                      <div>
                        <div className="flex items-center gap-3 text-[#6b7b65] font-mono text-[10px] tracking-widest uppercase mb-3"><MapPin size={14}/> Office Address</div>
                        <p className="text-neutral-600 font-light text-sm leading-relaxed whitespace-pre-line">{loc.address}</p>
                      </div>
                      <div>
                        <div className="flex items-center gap-3 text-[#6b7b65] font-mono text-[10px] tracking-widest uppercase mb-3"><Clock size={14}/> Operating Hours</div>
                        <p className="text-neutral-600 font-light text-sm leading-relaxed">{loc.hours}</p>
                      </div>
                    </div>
                    <div className="space-y-6">
                      <div>
                        <div className="flex items-center gap-3 text-[#6b7b65] font-mono text-[10px] tracking-widest uppercase mb-3"><Phone size={14}/> Contact Numbers</div>
                      ...
                      {loc.email && (
                        <div>
                          <div className="flex items-center gap-3 text-[#6b7b65] font-mono text-[10px] tracking-widest uppercase mb-3"><Mail size={14}/> Email Address</div>
                          <a href={`mailto:${loc.email}`} className="text-sm font-medium text-[#111111] hover:text-[#6b7b65] transition-colors block break-all">{loc.email}</a>
                        </div>
                      )}
                    </div>
                  </div>
